...
with base as (
    ...
    select *
    from {{ ref('stg_taboola__content_provider_platform_report_tmp') }}
),
    ...
    select
        {{
            fivetran_utils.fill_staging_columns(
                source_columns = adapter.get_columns_in_relation(ref('stg_taboola__content_provider_platform_report_tmp')),
                staging_columns = get_content_provider_platform_report_columns()
            )
        }}
    from base
),
...
final as (
    ...
    select
        ...
        {{ fivetran_utils.fill_pass_through_columns('taboola__content_provider_platform_report_passthrough_columns') }}
