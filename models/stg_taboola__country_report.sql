...
with base as (
    ...
    select *
    from {{ ref('stg_taboola__country_report_tmp') }}
),
    ...
    select
        {{
            fivetran_utils.fill_staging_columns(
                source_columns = adapter.get_columns_in_relation(ref('stg_taboola__country_report_tmp')),
                staging_columns = get_country_report_columns()
            )
        }}
    from base
),
...
final as (
    ...
    select
        ...
        {{ fivetran_utils.fill_pass_through_columns('taboola__country_report_passthrough_columns') }}
